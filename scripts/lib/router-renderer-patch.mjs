...
const RRouterProviders=[
  {value:"rox",label:"ROX",description:"Use api.rox.one without a Cursor login.",kind:"key",secret:"OMNIROUTE_API_KEY"},
...
function RRouterState(){
  const[s,e]=de.useState({provider:"rox",usage:null,local:null,error:null});
...
function RBoxRuntime(){const[s,e]=de.useState({mode:"m4697",status:null,error:null,busy:!0});de.useEffect(()=>{let t=!0;window.desktop.agent.getBoxRuntime().then(n=>{t&&e({...n,error:null,busy:!1})}).catch(n=>{t&&e(r=>({...r,error:String(n?.message??n),busy:!1}))});return()=>{t=!1}},[]);const t=s.mode==="local-docker",n=s.mode==="m4697",r=async()=>{const i=t?"m4697":"local-docker";e(o=>({...o,mode:i,busy:!0,error:null}));try{const o=await window.desktop.agent.setBoxRuntime(i);e({...o,error:null,busy:!1})}catch(o){e(l=>({...l,mode:t?"local-docker":"m4697",error:String(o?.message??o),busy:!1}))}};return a.jsxs("div",{children:[a.jsx(ie,{description:n?(s.status?.detail??"Browsers, Shell and files run on dedicated server m4697."):t?(s.status?.detail??"Shell, files and computer use run in a Docker container on this Mac."):"Shell, files and computer use run on Grok Bot's remote computer.",label:n?"Dedicated server m4697":"Use local Docker VM",variant:"card",children:a.jsx("button",{"aria-checked":t,"aria-label":"Use local Docker VM",disabled:s.busy,onClick:r,role:"switch",style:{appearance:"none",background:t?"var(--color-accent-primary)":"transparent",border:"1px solid currentColor",borderRadius:999,height:22,width:40},type:"button"})})]})}
function RRouterModel(){const[s,e]=de.useState({models:[{id:"gpt-5.6-luna"}],modelId:"gpt-5.6-luna",effort:"medium",agentId:null,error:null,busy:!0});de.useEffect(()=>{let t=!0;(async()=>{try{const[n,r]=await Promise.all([window.desktop.agent.listRoxModels().then(i=>i).catch(()=>({models:[{id:"gpt-5.6-luna"}]})),window.desktop.agent.getAgentRoxModel().then(i=>i).catch(()=>({modelId:"gpt-5.6-luna",effort:"medium",agentId:null}))]);const i=Array.isArray(n)?n:Array.isArray(n?.models)?n.models:[{id:"gpt-5.6-luna"}],o=i.some(l=>l.id==="gpt-5.6-luna")?i:[{id:"gpt-5.6-luna"},...i];t&&e({models:o.length?o:[{id:"gpt-5.6-luna"}],modelId:r?.modelId||"gpt-5.6-luna",effort:r?.effort||"medium",agentId:r?.agentId??null,error:null,busy:!1})}catch(n){t&&e(r=>({...r,error:String(n?.message??n),busy:!1}))}})();return()=>{t=!1}},[]);const t=async n=>{e(r=>({...r,...n,busy:!0,error:null}));try{const r=await window.desktop.agent.setAgentRoxModel({agentId:s.agentId,modelId:n.modelId??s.modelId,effort:n.effort??s.effort});e(i=>({...i,...r,busy:!1,error:null}))}catch(r){e(i=>({...i,busy:!1,error:String(r?.message??r)}))}};const n=s.models.map(r=>({value:r.id,label:r.id})),r=[{value:"none",label:"none"},{value:"low",label:"low"},{value:"medium",label:"medium"},{value:"high",label:"high"}];return a.jsxs("div",{children:[a.jsx(ie,{description:"Live ids from api.rox.one. Default is gpt-5.6-luna.",label:"Model",variant:"card",children:a.jsx(ye,{"aria-label":"ROX model",disabled:s.busy,onValueChange:i=>{if(i!==null)void t({modelId:i})},options:n,placement:"bottom-end",size:"lg",value:s.modelId,variant:"filled"})}),a.jsx(ie,{divided:!0,description:"Reasoning effort. Default is medium.",label:"Effort",variant:"card",children:a.jsx(ye,{"aria-label":"Reasoning effort",disabled:s.busy,onValueChange:i=>{if(i!==null)void t({effort:i})},options:r,placement:"bottom-end",size:"lg",value:s.effort,variant:"filled"})}),s.error?a.jsx(se,{as:"p",color:"red",size:"sm",children:s.error}):null]})}
function RRouterPanel(){const[s,e]=RRouterState(),[t,n]=RRouterSecrets(),r=RRouterProviders.find(i=>i.value===s.provider)??RRouterProviders[0],i=s.usage?.providers?.[s.provider]??RRouterEmptyUsage,o=r.value==="codex"?"Uses the private ChatGPT login already stored by Codex on this Mac. Requests are made by Grok Bot directly.":r.kind==="local"?"Uses Claude Code's existing login on this Mac.":r.kind==="key"?"Stored securely with your other Grok Bot secrets.":"Uses the account already connected to Grok Bot.";return a.jsx(Te,{children:a.jsxs("div",{className:k("sand-settings-general","sand-9f619 sand-78zum5 sand-dt5ytf sand-3qzy4x"),children:[a.jsx(re,{title:"Routing",children:a.jsx(ie,{description:r.description,label:"Provider",variant:"card",children:a.jsx(ye,{"aria-label":"Routing provider",onValueChange:l=>{if(l!==null)void e(l)},options:RRouterOptions,placement:"bottom-end",size:"lg",value:s.provider,variant:"filled"})})}),a.jsx(re,{title:"Model",children:a.jsx(RRouterModel,{})}),a.jsx(re,{title:"Computer",children:a.jsx(RBoxRuntime,{})}),a.jsx(re,{title:r.kind==="key"?"OpenRouter account":"Account",children:a.jsx(ie,{description:o,label:r.kind==="key"?"API key":"Status",variant:"card",children:a.jsx(RRouterCredential,{provider:r,state:s,keys:t,onSaved:n})})}),s.error?a.jsx(se,{as:"p",color:"red",size:"sm",children:s.error}):null,a.jsx(re,{title:"Usage for "+r.label,children:a.jsx(RRouterUsageRows,{usage:i})})]})})}
...
export async function applyOriginalRendererRouterPatch({ stageRoot }) {
  ...
  const record = {
    ...
    chunks: changes,
    features: ["settings-router-provider", "settings-router-model", "settings-local-docker-vm", "usage-current-provider"],
